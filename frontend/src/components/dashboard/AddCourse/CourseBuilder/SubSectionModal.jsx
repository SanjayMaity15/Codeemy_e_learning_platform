...
export default function SubSectionModal({
  ...
  const handleEditSubsection = async () => {
    ...
    setLoading(true)
    const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}course/updateSubSection`, formData, { withCredentials: true })
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true)
    const response = await axios.post(`${import.meta.env.VITE_SERVER_URL}course/addSubSection`, formData, { withCredentials: true })
				...
				>
					...
					<div className="flex flex-col space-y-2">
						...
						<textarea
							...
							className="form-style resize-x-none min-h-32.5 w-full p-1 bg-white px-4 rounded-2xl focus:outline-primary outline"
						/>
						{errors.lectureDesc && (
							<span className="ml-2 text-xs tracking-wide text-red-600">
